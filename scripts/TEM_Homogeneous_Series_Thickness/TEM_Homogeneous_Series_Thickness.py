...
# Simulate EDS spectra of a TEM sample with homogeneous composition for multiple TEM sample thicknesses.
#
...
# MANUAL
# thicknesses = [10, 20, 50, 100] # thickness, in nm
...
thicknesses = [start + x * (end - start) / steps for x in range(steps + 1)]

# beam / acquisition setup
e0 = 200.0  # keV (primary beam energy)
pc = 1.0  # nA (probe current)
lt = 5.0  # sec (live time for spectrum collection)
dose = pc * lt  # nA*sec
...
nTraj = 2000  # number of trajectories to run
...
trajectory_img_same_field_of_view = False  # if True, same field of view will be used for overview images of trajectories
...
xrts = mc.suggestTransitions(sample, e0)


for i, thickness in enumerate(thicknesses):
    print("-----------------------------------------------------------")
    print(
        "Simulating sample thickness: "
        + str(thickness)
        + " nm ("
        + str(i + 1)
        + "/"
    ...
    if trajectory_img_same_field_of_view:
        imgSzNm = max(thicknesses) * 2
    xtraParams = {}
    xtraParams.update(
