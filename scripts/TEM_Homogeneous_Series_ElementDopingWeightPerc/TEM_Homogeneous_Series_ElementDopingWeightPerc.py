...
# Simulate EDS spectra of a TEM sample with homogeneous composition for different doping levels (wt%) of a single element.
# 
...
# sample setup
sample = material("Hf6Ta2O17", 5.0)  # Material
...
# Dopant value in !!weight percent!! added to the sample stoichiometry
...
# MANUAL
df = [0.1, 1.0, 5.0]  # Dopant value in !!weight percent!!
...
for i, f in enumerate(df):
    ...
    # add one weight percent of element to the sample and calculate required weight percentage to ADD for specific TOTAL weight fraction
    sample_tmp.addElement(dopant, 1.0)
    f_onePerc = sample_tmp.weightFraction(element(dopant), True)

    # we calculate the required weight fraction to ADD for a desired TOTAL weight percentage of doping
    f_toAdd = f / f_onePerc * (1.0 - f_onePerc) / (1 - f)

    # modify original sample by adding dopant, actually build the doped sample
    sample_tmp = sample.clone()
    sample_tmp.addElement(dopant, f_toAdd)
