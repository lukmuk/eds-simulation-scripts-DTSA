...
# Simulate EDS spectra of a TEM sample with homogeneous composition for multiple total electron doses.
# Here, the live time is adjusted (live_times) for a given probe current (pc) and the total doses are stored in "doses"
#
...
# MANUAL
live_times = [0.01, 0.1, 1, 10] # Live time, in s
...
#live_times = [start + x*(end-start)/steps for x in range(steps+1)]

# beam / acquisition setup
e0 = 80.0  # keV (primary beam energy)
...
doses = [pc * lt for lt in live_times]  # nA*sec
...
for i, dose in enumerate(doses):
    print("-----------------------------------------------------------")
    print("Dose: " + str(dose) + " nAs (" + str(i + 1) + "/" + str(len(doses)) + ")")
    print("-----------------------------------------------------------")
    xtraParams = {}
    xtraParams.update(
