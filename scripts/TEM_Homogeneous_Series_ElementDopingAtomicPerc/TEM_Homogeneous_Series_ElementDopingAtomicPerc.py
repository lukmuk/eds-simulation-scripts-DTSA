...
#
# Author: Lukas Gruenewald, last updated: 27.09.24
...
# sample setup
sample = material("ZnO", 5.0)  # Material
dopant = element("S")  # dopant element
thickness = 100  # nm (TEM sample thickness)

# series setup (define as manual list or linear array in list)
# Dopant value in !!atomic percent!! added to the sample stoichiometry
...
# MANUAL
df = [0.1, 1.0, 5.0]  # Dopant value in !!atomic percent!!
...
# Convert from at% to fractions
df = [f / 100 for f in df]

# specify the extra parameters based on user parameters
...
for i, f in enumerate(df):
    ...
    # modify original sample by adding dopant, this is first to calculate the required number of atoms
    sample_tmp = sample.clone()

    # add one atom of element to the sample and calculate required number of atoms to add for specific atomic fraction
    sample_tmp.addElementByStoiciometry(dopant, 1.0)
    df_oneAtom = sample_tmp.atomicPercent(element(dopant))

    # we calculate the required number of atoms to add for a desired atomic percent of doping
    f_doping_atoms = f / df_oneAtom * (1.0 - df_oneAtom) / (1 - f)

    # modify original sample by adding dopant, actually build the doped sample
    sample_tmp = sample.clone()
    sample_tmp.addElementByStoiciometry(dopant, f_doping_atoms)

    print("Stoichimetry (norm.):\t" + sample_tmp.stoichiometryString())
    print(
        "Total dopant (wt%):\t\t"
        + str(sample_tmp.weightFraction(element(dopant), True) * 100)
    )
    print(
        "Total dopant (at%):\t\t" + str(sample_tmp.atomicPercent(element(dopant)) * 100)
    )
    print("-----------------------------------------------------------")
