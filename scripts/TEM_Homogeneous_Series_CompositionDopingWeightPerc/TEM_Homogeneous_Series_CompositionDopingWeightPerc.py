...
# Simulate EDS spectra of a TEM sample with homogeneous composition for different doping levels (wt%) of an added composition/material.
#
...
# sample setup
density = 5.0  # sample density
sample = material("TiO2", density)  # Material
dopant = material("Si3N4", density)  # dopant material
...
# MANUAL
df = [0.5, 2.0]  # Dopant value in !!weight percent!!
...
for i, f in enumerate(df):
    ...
    c = epq.Composition()
    c.defineByMaterialFraction([sample, dopant], [1 - f, f])
    sample_tmp = material(c, density)
    sample_tmp.setName("DopeComp")

    print("Stoichimetry (norm.):\t" + c.stoichiometryString())
    # print("Total dopant (wt%):\t\t"+str(c.weightFraction(element(dopant), True)*100))
    # print("Total dopant (at%):\t\t"+str(c.atomicPercent(element(dopant))*100))
    print("-----------------------------------------------------------")
